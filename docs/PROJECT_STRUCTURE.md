...
│  ├─ console-logger.js                    # 控制台日志分隔、类型标注与统一输出格式
│  ├─ debug-logger.js                      # 全局 debug 开关与本地 JSONL 日志
│  ├─ env.js                               # 轻量 .env 加载器
...
├─ config.json                             # debug、浏览器、页面文本、重试和上限配置
├─ .env.example                            # debug 和百炼环境变量模板
├─ package.json                            # 命令与依赖
...
- `src/debug-logger.js`
  - 读取全局 `debug` / `XHS_DEBUG` 开关。
  - 仅在开关开启时写入 `logs/debug-YYYY-MM-DD.log`，关闭时不创建 debug 日志。
  - 对 API Key、Authorization 和超大 base64 图片做本地日志脱敏与摘要。
- `src/console-logger.js`
  - 统一包装命令入口的 `console.log/info/warn/error/debug` 输出。
  - 每条控制台日志前后输出空行，并标注日志类型和内容，便于在大量导出日志中辨认。
- `src/open-browser.js`
...
- `src/bailian-client.js`
  ...
  - 当全局 debug 开启时，记录 AI 请求前、响应后、解析结果和异常路径的详细本地日志。
...
- `public/index.html`
  - 七个视图：生命周期、发布时间、内容漏斗、封面、笔记横向对比、内容策略、内容实验室（从内容实验建议进入）。
...
- `debug`：全局本地 debug 日志开关，默认 `false`。
...
- `bailian-client.test.js`：策略建议的新旧 JSON 字段归一化。
- `debug-logger.test.js`：debug 日志脱敏和大字段摘要。
- `console-logger.test.js`：控制台日志类型标签和内容格式。
